# -*-coding:utf-8-*-
import torch
from utilize.dirpath import sky_block4, iter_1_save_path, generic_save_path, \
...
path_new = '/sky_100000/block4/our_method'
...
for i in range(len(save_path)):
    ...
    generate_main_diversity(sky_block4, Now_model[i], device, save_path[i], NUMBER, top_k, l, sentence_num)
    extract_accept(save_path[i], limit, save_path)
    extract_zip_accept(save_path[i])
    extract_name(save_path[i], name_save_path[i])
    reconstruct_main(device, sky_block4, name_save_path[i], reconstruct_save_path[i])
    train_and_save(Next_model[i], Now_model[i], PATHS[i:i+1], BLOCK_SIZE, BATCH_SIZE, EPOCHS, device, num)
